// (c) Jisin0
// Hybrid IMDb Direct + TMDB/OMDb Fallback Engine.
...
// SearchIMDbDirect: Tries GraphQL first; returns error if blocked or empty.
...
// SearchOMDb: Primary IMDb Direct Search with TMDB Multi Fallback.
...
// 1. Try Direct IMDb Search
if res, err := SearchIMDbDirect(query); err == nil && len(res) > 0 {
return res, nil
}

// 2. Fallback to TMDB Multi Search
...
// Fetch TMDB rich metadata & credits
var t tmdbDetail
if tmdbID != "" {
...
// Resolve IMDb ID for OMDb lookup
imdbID := directImdbID
...
// 1. Official IMDb Rating and Votes
ratingLine := ""
